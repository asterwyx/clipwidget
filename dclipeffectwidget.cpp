...
bool DClipEffectWidget::eventFilter(QObject *watched, QEvent *event)
{
    ...
    if (event->type() == QEvent::Paint) {
        ...
        int rawMaxError = d->margins.isNull() ? 1 : 0;

        d->imageGeometry = QRectF(image.rect()) & multiply(QRect(offset, size()).adjusted(0, 0, rawMaxError, rawMaxError), scale);
...
void DClipEffectWidget::paintEvent(QPaintEvent *event)
{
    ...
    int rawMaxError = d->margins.isNull() ? 1 : 0;
    int scaledMaxError = rawMaxError * qRound(devicePixelRatio);
    const QRectF &rect = QRectF(event->rect()) & QRectF(this->rect().marginsRemoved(d->margins));
    const QPoint &offset = mapTo(window(), QPoint(0, 0));
    const QRectF &imageRect = multiply(rect, devicePixelRatio) & d->imageGeometry.translated(-QPointF(offset) * devicePixelRatio);
    ...
    newPath.addRect(this->rect().adjusted(0, 0, rawMaxError, rawMaxError));
    newPath -= d->path;

    p.setRenderHint(QPainter::Antialiasing);
    p.setClipPath(newPath);
    p.setCompositionMode(QPainter::CompositionMode_Source);
    p.drawImage(d->margins.isNull() ? QPoint(0, 0) : imageRect.topLeft() / devicePixelRatio, d->image, imageRect.adjusted(0, 0, scaledMaxError, scaledMaxError));
}
